...
def main():
    ...
    for row in rows:
        ...
        if schedule:
            try:
                ...
                print(f"[WARN] duration計算失敗 schedule={schedule}: {e}")
        ...
        if day and schedule:
            try:
                ...
                print(f"[WARN] day_number計算失敗 day={day}: {e}")
    ...
    # 住之江の二重（電話投票テーブル）など重複があれば末尾を落とす（元コード踏襲）
    if not df_index_subset.empty and df_index_subset["place"].duplicated().any():
    ...
    # code を文字列として扱う（race_id 生成で桁が崩れないように）
    race_merged["code"] = race_merged["code"].astype(str).str.zfill(2)
    code_list = race_merged["code"].tolist()

    print(f"[INFO] 開催場数: {len(code_list)}")

    # ---------- 各会場 × 12R ----------
    for code in code_list:
        for rno in tqdm(range(1, 13), desc=f"code={code}"):
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        mark_list = [conversion.get(v, "0") for v in mark_list]
                        if len(mark_list) != 6:
                            mark_list = ["0"] * 6
                    ...
                    # ここは元コード踏襲（iterrowsでの加工）
                    for _, row_ in ex_entry.iterrows():
                    ...
                    num = []
                    ex_entry_member = pd.merge(ex_entry, ex_member, on="entry", how="left").drop(
                        ["コース", "並び", "ST"], axis=1, errors="ignore"
                    )
                    ...
                    if re.match(r"水面気象情報\s0:00現在", observation_time):
                        weather = soup_b.find("div", {"class": "weather1_bodyUnit is-weather"}).find(
                            "span", {"class": "weather1_bodyUnitLabelTitle"}
                        ).text.strip()
                        ...
                        temperature = soup_b.find("div", {"class": "weather1_bodyUnitLabel"}).find(
                            "span", {"class": "weather1_bodyUnitLabelData"}
                        ).text.strip()
                        weather = soup_b.find("div", {"class": "weather1_bodyUnit is-weather"}).find(
                            "span", {"class": "weather1_bodyUnitLabelTitle"}
                        ).text.strip()
                        wind_speed = soup_b.find("div", {"class": "weather1_bodyUnit is-wind"}).find(
                            "span", {"class": "weather1_bodyUnitLabelData"}
                        ).text.strip()
                        water_temperature = soup_b.find("div", {"class": "weather1_bodyUnit is-waterTemperature"}).find(
                            "span", {"class": "weather1_bodyUnitLabelData"}
                        ).text.strip()
                        wave_height = soup_b.find("div", {"class": "weather1_bodyUnit is-wave"}).find(
                            "span", {"class": "weather1_bodyUnitLabelData"}
                        ).text.strip()

                        wind_direction_class = soup_b.find("div", {"class": "weather1_bodyUnit is-windDirection"}).find("p")[
                            "class"
                        ]
                    ...
                    df_r[2]["ST"] = df_r[2]["スタート情報"].str[3:7].map(lambda x: "0" + str(x).strip())
                    ...
                    num = [e + 1 for e in range(len(entry))]
    ...
    if refund_pickles:
        ...
        refund.to_csv(out_refund, index=False, encoding="utf-8-sig")
